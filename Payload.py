...
def detect_payload_datatype(data: Any) -> Optional[str]:
    ...
    # JSON
    if get_datatype(data) == {Datatype.JSON}:
    
        json_data = json.loads(data)  # Parse JSON
        payload = get_payload()  # Get new payload

        # Replace all values in JSON with payload
        def replace_all(obj):
            if isinstance(obj, dict):
                return {key: replace_all(value) for key, value in obj.items()}
            elif isinstance(obj, list):
                return [replace_all(value) for value in obj]
            else:
                return str(payload)  # Replace value with payload

        new_json = replace_all(json_data)  # Modify JSON
        return json.dumps(new_json) # Convert JSON back to string
...
if __name__ == "__main__":
    ...
    json_data = '''
    {
        ...
        "tags": ["python", "security", "xss"]
    }
    '''
    modified_json_data = detect_payload_datatype(json_data)
    ...
    # print("==== Testing PHP Serialized Data ====")
    # php_serialized_data = 'a:2:{s:5:"name";s:5:"Alice";s:5:"email";s:15:"alice@example.com";}'
    # modified_php_data = detect_payload_datatype(php_serialized_data)
    # print("Original:", php_serialized_data)
    # print("Modified:", modified_php_data)
    # print("-" * 50)

    # # Test img data
    # print("==== Testing Image Filename Data ====")
    # image_filename = "profile.jpg"
    # modified_image = detect_payload_datatype(image_filename)
    # print("Original:", image_filename)
    # print("Modified:", modified_image)
    # print("-" * 50)