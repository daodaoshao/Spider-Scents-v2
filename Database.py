...
class Database(object):
    ...
    def get_table_dependencies(self) -> dict:
        """Get table dependencies as a directed graph.
        - If foreign keys exist, build edges based on actual foreign keys.
        - If no foreign keys exist, return an isolated graph where tables are nodes but no edges exist.

        Returns:
            dependency_graph (dict): A dictionary where keys are tables
                                    and values are lists of dependent tables.
        """
        cursor = self.connection.cursor()
        ...
        # Initialize the dependency graph with all tables as nodes (default empty list)
        dependency_graph = {table: [] for table in tables}

        # Step 2: Try to find actual foreign key relationships
        query = """
        ...
        # Step 3: If foreign keys exist, build edges in the graph
        for table, ref_table in results:
            if ref_table in dependency_graph:
                dependency_graph[ref_table].append(table)
    ...
    def bfs_traversal(self) -> list[str]:
        """
        Use BFS (Breadth First Search) to traverse the database tables, prioritizing dependent tables.
        Ensures all tables are included in the traversal even if they have no dependencies.

        Returns:
            list[str]: The traversal order of tables.
        """
        dependency_graph = self.get_table_dependencies()

        # Step 1: Compute in-degrees (number of incoming edges)
        in_degree = {table: 0 for table in dependency_graph}
        for parent, children in dependency_graph.items():
            for child in children:
                in_degree[child] += 1

        # Step 2: Initialize BFS queue with all tables having in-degree = 0
        queue = deque([table for table in in_degree if in_degree[table] == 0])
        traversal_order = []

        while queue:
            table = queue.popleft()
            traversal_order.append(table)

            # Reduce the in-degree of dependent tables
            for dependent_table in dependency_graph.get(table, []):
                in_degree[dependent_table] -= 1
                if in_degree[dependent_table] == 0:
                    queue.append(dependent_table)

        # Step 3: Handle isolated tables (not included in BFS traversal)
        for table in dependency_graph:
            if table not in traversal_order:
                traversal_order.append(table)
    ...
    def dfs_traversal(self) -> list[str]:
        """
        Use DFS (Depth First Search) to traverse the database tables, prioritizing dependent tables.
        """
        dependency_graph = self.get_table_dependencies()
        visited = set()
        traversal_order = []
        ...
        def dfs(table):
            ...
            for dependent_table in sorted(dependency_graph.get(table, [])):
                dfs(dependent_table)


        
        for table in sorted(dependency_graph.keys()):  # sort to ensure consistent order
            if table not in visited:
                dfs(table)

        return traversal_order
    
    def get_sorted_tables(self, traversal_method="BFS") -> list[str]:
        """
        Get the tables in the database in a sorted order.
        """

        if traversal_method.upper() == "BFS":
            return self.bfs_traversal()
        elif traversal_method.upper() == "DFS":
            return self.dfs_traversal()
        else:
    ...
    def get_columns_info(self, table: str) -> list[ColInfo]:
        ...
        def process(column: tuple) -> ColInfo:
            ...
            values = []
            if isinstance(longtype, bytes):
                longtype = longtype.decode('utf-8')
            if type in {'enum', 'set'}:
    ...
    def generate_row(col_info: list[ColInfo], i: int = 0) -> tuple:
        ...
        for _, type, _, values in col_info:
            ...
            elif Database.is_type_json(type):
                # TODO this is not a good way to generate json data
                # but it works for now
                row.append(f'{{"example": {i}}}')
            else:
...
if __name__ == "__main__":
    ...
    method = "DFS"
    dependency_graph = db.get_table_dependencies()
    print(f"Database table dependencies: {dependency_graph}")
    sorted_tables = db.get_sorted_tables(traversal_method=method)
    print(f"Database tables sorted using {method}: {sorted_tables}")
